      ...
      <div className="mx-auto max-w-6xl">
        ...
        <div className="flex flex-col" style={{ perspective: "1400px", perspectiveOrigin: "50% 0%" }}>
          {AGENTS.map((agent, index) => (
            <div key={agent.name} className="sticky mb-4" style={{ top: 80 + index * 12, zIndex: 10 + index }}>
              <div className="group relative cursor-pointer overflow-hidden rounded-2xl border border-white/[0.12] bg-[rgba(18,16,16,0.62)]">
                <div className="relative h-52 w-full overflow-hidden">
                  <img src={agent.image} alt={agent.name} className="h-full w-full object-cover" />
                  <div className="absolute inset-0 bg-gradient-to-t from-[rgba(12,10,10,0.95)] via-[rgba(12,10,10,0.35)] to-transparent" />
                  <span className="absolute left-6 top-6 font-pixel text-[10px] tracking-[0.28em] text-white/70">
                    {agent.name}
                  </span>
                </div>
                <div className="grid gap-6 px-6 py-6 md:grid-cols-[1fr_auto] md:items-end">
                  <div>
                    ...
                    <p className="max-w-xl text-sm leading-relaxed text-white/45">{agent.body}</p>
                  </div>
